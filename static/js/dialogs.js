/* ============================================================
   dialogs.js — Detail panel, dialogs, preview, delete, recycle
   ============================================================ */
...
function extractFile(el){
  ...
  fetch('/api/extract',{method:'POST',headers:{'Content-Type':'application/json','X-DocLib-Token':token},body:JSON.stringify({work_path:c.work_path,target_dir:td})})
...
// ======== BATCH RECYCLE ========
let _deleteUseBatch=false;

function batchRecycle(){
  const items=[...selectedIds].map(wp=>itemByPath.get(wp)).filter(Boolean);
  if(!items.length) return;
  _deleteUseBatch=true;
...
async function confirmDeleteAction(){
  ...
  if(!items.length) return;
  const useBatch=_deleteUseBatch||(items.length>1);
  ...
  try{
    if(useBatch||items.length>1){
      const r=await fetch('/api/batch-delete',{
        method:'POST',
        headers:{'Content-Type':'application/json','X-DocLib-Token':token},
        body:JSON.stringify({items:items.map(c=>({work_path:c.work_path}))})
      });
      const d=await r.json();
      d.recycled.forEach(wp=>{
        itemByPath.delete(wp);
        selectedIds.delete(wp);
        favoritesSet.delete(wp);
      });
      allItems=allItems.filter(it=>!d.recycled.includes(it.work_path));
      if(d.recycled.includes(activeDetailPath)) closeDetail();
      applyFilters();
...
function showDetail(wp){
  ...
  let h='';
  [['周次',c.week],['分类',CAT_NAMES[c.cat_num]],['来源',c.source||'--'],
  ...
  h+=`<div class="panel-actions">
    <button class="btn-sm btn-open" style="font-size:13px;padding:6px 14px" data-wp="${esc(wp)}" onclick="previewFile(this)">预览</button>
    <button class="btn-sm btn-extract" style="font-size:13px;padding:6px 14px" data-wp="${esc(wp)}" onclick="openFile(this)">系统打开</button>
    <button class="btn-sm btn-extract" style="font-size:13px;padding:6px 14px" data-wp="${esc(wp)}" onclick="extractFile(this)">提取</button>
    <button class="btn-sm" style="font-size:13px;padding:6px 14px;background:var(--badge-bg);color:var(--text);border:1px solid var(--border)" onclick="toggleFav('${esc(wp)}');showDetail('${esc(wp)}')">${fav?'取消收藏':'收藏'}</button>
    <button class="btn-sm btn-danger" style="font-size:13px;padding:6px 14px" data-wp="${esc(wp)}" onclick="confirmDelete(this)">回收</button>
  </div>`;
...
async function openFile(el){
  ...
  try{
    const r=await fetch('/api/open',{method:'POST',headers:{'Content-Type':'application/json','X-DocLib-Token':token},body:JSON.stringify({work_path:c.work_path})});
...
async function startBatchExtract(){
  ...
  try{
    const r=await fetch('/api/batch-extract',{
      method:'POST',headers:{'Content-Type':'application/json','X-DocLib-Token':token},
