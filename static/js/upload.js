/* ============================================================
   upload.js — File upload (drag/drop, select, XHR progress) (ES module)
   ============================================================ */

import { state } from './state.js';
import { esc, fmtSize, showToast } from './utils.js';
...
export function addUploadFiles(fileList) {
  ...
  for (let i = 0; i < fileList.length; i++) {
    ...
    if (file.size > state.MAX_UPLOAD_PER_FILE) { skippedSize++; continue; }
    if (state.uploadFiles.some(function (f) { return f.name === file.name; })) { skippedDup++; continue; }
...
export function refreshUploadQueue() {
  ...
  if (state.uploadFiles.length === 0) {
    ...
    for (let i = 0; i < state.uploadFiles.length; i++) {
      totalSize += state.uploadFiles[i].size;
      html += '<div style="font-size:12px;padding:5px 8px;margin:2px 0;background:var(--badge-bg);border-radius:4px;display:flex;justify-content:space-between;align-items:center">' +
        '<span><b>' + esc(state.uploadFiles[i].name) + '</b> <span style="color:var(--text2)">' + fmtSize(state.uploadFiles[i].size) + '</span></span>' +
        '<a style="color:var(--accent);cursor:pointer;font-weight:bold;font-size:15px" onclick="DL.removeUploadFile(' + i + ')">&times;</a>' +
        '</div>';
    }
    listEl.innerHTML = html;
    listEl.style.display = 'block';
    btn.disabled = false;
    dropText.innerHTML = '<b>' + state.uploadFiles.length + '</b> 个文件 &middot; 共 ' + fmtSize(totalSize);
  }
}
...
export function startUpload() {
  ...
  textEl.textContent = '准备上传 ' + state.uploadFiles.length + ' 个文件...';

  const formData = new FormData();
  for (let i = 0; i < state.uploadFiles.length; i++) {
    formData.append('files', state.uploadFiles[i]);
  }

  const xhr = new XMLHttpRequest();
  xhr.open('POST', '/api/upload');
  xhr.setRequestHeader('X-DocLib-Token', state.token);
  ...
  xhr.onload = function () {
    try {
      ...
      setTimeout(function () { window.DL.init(); closeUploadDialog(); }, 2000);
    } catch (ex) {
