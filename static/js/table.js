/* ============================================================
   table.js — Virtual scrolling table, column resize, selection
   ============================================================ */
...
function renderTable(){
  ...
  let start=Math.max(0,Math.floor(top/ROW_H)-BUFFER),
      end=Math.min(filteredItems.length,Math.ceil((top+viewH)/ROW_H)+BUFFER);

  rows.style.transform=`translateY(${start*ROW_H}px)`;

  let html='';
  for(let i=start;i<end;i++){
    const c=filteredItems[i], wp=c.work_path,
          sel=selectedIds.has(wp),
          fav=favoritesSet.has(wp),
          act=wp===activeDetailPath;
    ...
    html+=`<div class="table-row${sel?' selected':''}${act?' active-detail':''}" data-wp="${esc(wp)}">
      ...
      <div class="cell actions">
        ...
        <button class="btn-sm btn-open" data-wp="${esc(wp)}" onclick="event.stopPropagation();previewFile(this)" title="在浏览器中预览">预览</button>
        <button class="btn-sm btn-extract" data-wp="${esc(wp)}" onclick="event.stopPropagation();extractFile(this)" title="复制到提取目录">提取</button>
        <button class="btn-sm btn-danger" data-wp="${esc(wp)}" onclick="event.stopPropagation();confirmDelete(this)" title="移至回收站">回收</button>
      </div>
    </div>`;
  }
...
function onScroll(){
  ...
  rafId=requestAnimationFrame(renderTable);
}
...
// ======== SELECTION ========
function toggleSel(wp){
  selectedIds.has(wp)?selectedIds.delete(wp):selectedIds.add(wp);
  updateSelection();
  renderTable();
}
...
function updateSelection(){
  ...
  document.getElementById('selectAll').checked=cnt>0&&cnt===filteredItems.length&&filteredItems.length>0;
  document.getElementById('clearSelBtn').style.display=cnt>0?'':'none';
}

function updateStatusBar(){
  document.getElementById('totalCount').textContent=allItems.length.toLocaleString()+' 项';
  document.getElementById('filteredCount').textContent=filteredItems.length.toLocaleString()+' 项';
}
